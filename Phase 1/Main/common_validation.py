"""Common validation and rate limiting utilities (Phase 1 refactor)

Unifies previously duplicated RateLimiter and FlightSearchValidator implementations.
Adds price parsing helpers.
"""
...
class RateLimiter:
    """Simple in-memory rate limiter (minute + hour windows)."""
    def __init__(self):
    ...
    def reset(self) -> None:  # useful for tests
        self._minute.clear()
        self._hour.clear()

class FlightSearchValidator:
    """Unified validation with configurable horizon / infant rule enforcement."""
    @staticmethod
    def validate_airport_code(code: str) -> bool:
        if not code:
            return False
        return (len(code) == VALIDATION_RULES['airport_code_length'] and code.isalpha() and code.isupper())

    @staticmethod
    def validate_date(date_str: str, enforce_horizon: bool = True) -> bool:
        """Validate date format and (optionally) horizon using shared util."""
        try:
            datetime.strptime(date_str, '%Y-%m-%d')
        except ValueError:
            return False
        if not enforce_horizon:
            return True
    ...
    def validate_passengers(adults: int, children: int, infants_seat: int, infants_lap: int, enforce_infant_rule: bool = True) -> bool:
        ...
        if adults < 1:
            return False
        if total > VALIDATION_RULES['max_passengers']:
            return False
        if any(x < 0 for x in [adults, children, infants_seat, infants_lap]):
            return False
        if enforce_infant_rule and infants_lap > adults:
            return False
        return True
    ...
    def validate_search_params(params: Dict[str, Any], *, enforce_horizon: bool = True, enforce_infant_rule: bool = True) -> Tuple[bool, List[str]]:
        ...
        # Additional logical check: return_date >= outbound_date
        if params.get('outbound_date') and params.get('return_date'):
            try:
                od = datetime.strptime(params['outbound_date'], '%Y-%m-%d')
                rd = datetime.strptime(params['return_date'], '%Y-%m-%d')
                if rd < od:
...
def parse_price(raw: Any) -> Tuple[Optional[int], Optional[str]]:
    """Parse price field that may look like '599 USD' -> (599, 'USD'). Returns (None, None) if unparseable."""
    if raw is None:
        return None, None
    if isinstance(raw, (int, float)):
        return int(raw), None
    s = str(raw).strip()
    m = _price_regex.match(s)
    if not m:
        return None, None
