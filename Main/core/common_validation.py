...
class FlightSearchValidator:
    ...
    def validate_passengers(adults: int, children: int, infants_seat: int, infants_lap: int, enforce_infant_rule: bool = True) -> bool:
        ...
        if (adults < 1 or total > VALIDATION_RULES['max_passengers'] or
                any(x < 0 for x in [adults, children, infants_seat, infants_lap])):
            return False
...
def parse_price(raw: Any) -> tuple[int | None, str | None]:
    if raw is None:
        return None, None
    if isinstance(raw, int | float):  # type: ignore[arg-type]
        return int(raw), None
    s = str(raw).strip()
